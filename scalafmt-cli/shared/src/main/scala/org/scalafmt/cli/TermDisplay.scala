...
object TermDisplay extends TermUtils {
  ...
  private class UpdateDisplayThread(
    ...
    private val q = CompatCollections.concurrentQueue[Message]
    ...
    val infos = CompatCollections.concurrentMap[String, Info]
      ...
      Option(q.poll(100L, TimeUnit.MILLISECONDS)) match {
        ...
        case Some(Message.Update) =>
          val downloads0 = downloads.synchronized {
            downloads.toVector
              .flatMap { url => infos.get(url).map(i => url -> i) }
              .sortBy { case (_, info) => -info.fraction.sum }
          }
...
class TermDisplay(
  ...
  override def taskProgress(url: String, downloaded: Long): Unit = {
    val info = updateThread.infos.get(url)
    if (info.isDefined) { // We might not want the progress bar.
      val newInfo = info match {
        case Some(info0: DownloadInfo) =>
